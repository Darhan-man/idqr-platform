...
async def generate_qr(request: Request, qrdata: str = Form(...), title: str = Form(...)):
    ...
    def draw_title_above_qr_dynamic(qr_img, title, font_path=FONT_PATH):
        ...
        font_size = 58
        font = None
        max_text_width = qr_width - 20 # 20px отступы с каждой стороны
        draw_temp = ImageDraw.Draw(Image.new('RGB', (1, 1)))

        lines = []
        # Шаг 1: Подбор размера шрифта и разбивка на строки
        while font_size > 10:
            try:
            ...
            full_text_bbox = draw_temp.multiline_textbbox((0, 0), '\n'.join(lines), font=font)
            text_height_total = full_text_bbox[3] - full_text_bbox[1]
            if text_height_total + 40 + qr_height < 1024: # Проверка, чтобы не превысить размер
                break
            font_size -= 2

        # Шаг 2: Расчет окончательного размера и отрисовка
        outline_thickness = 2
        line_spacing = 10
        top_padding = 20
        bottom_padding = 20
        
        # Получаем финальный размер текстового блока для точного расчета
        full_text_bbox = draw_temp.multiline_textbbox((0, 0), '\n'.join(lines), font=font)
        text_height_total = full_text_bbox[3] - full_text_bbox[1]
        
        final_width = qr_width
        final_height = qr_height + text_height_total + top_padding + bottom_padding

        final_img = Image.new("RGB", (final_width, int(final_height)), "white")
        draw_final = ImageDraw.Draw(final_img)

        y = top_padding
        for line in lines:
            line_bbox = draw_final.textbbox((0, 0), line, font=font)
            x = (final_width - (line_bbox[2] - line_bbox[0])) // 2
